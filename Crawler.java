...
public class Crawler extends WebCrawler {
	...
	public boolean shouldVisit(Page referringPage, WebURL url) {
		...
		if (href.startsWith("https://www.mercurynews.com/") || href.startsWith("http://www.mercurynews.com/")
				|| href.startsWith("https://mercurynews.com/") || href.startsWith("http://mercurynews.com/")) {
			urlok++;
	...
	public void visit(Page page) {
		...
		for (WebURL tempurl : page.getParseData().getOutgoingUrls()) {
			totalout++;
			if (tempurl.getURL().startsWith("https://www.mercurynews.com/")
					|| tempurl.getURL().startsWith("http://www.mercurynews.com/")
					|| tempurl.getURL().startsWith("https://mercurynews.com/")
					|| tempurl.getURL().startsWith("http://mercurynews.com/")) {// ||
				// tempurl.getURL().startsWith("https://www.nytimes.com/"))
				// {
				outurlok++;
...

	{
		...
		try {
			pwfetch = new PrintWriter(new File("fetch_MercuryNews.xslx"));
		...
		PrintWriter pw = null;
		try {
			pw = new PrintWriter(new File("visit_MercuryNews.xslx"));
		} catch (FileNotFoundException e) {
		...
		try {
			pw1 = new PrintWriter(new File("urls_MercuryNews.xslx"));
		} catch (FileNotFoundException e) {
		...
		pw1.close();
		System.out.println(" URLs within: " + urlok + " URLs outside: " + urlnok + " unique ok: " + uniqueok
				+ " uniquenok " + uniquenok + " unique " + unique);
		System.out.println(" URLs within: " + outurlok + " URLs outside: " + outurlnok + " unique ok: " + outuniqueok
				+ " uniquenok " + outuniquenok + " unique total  " + totalout);
		...
		System.out
				.println("File sizes: " + file1kb + " " + file10kb + " " + file100kb + " " + file1mb + " " + fileg1mb);
