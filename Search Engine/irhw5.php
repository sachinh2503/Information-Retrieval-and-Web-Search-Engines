...

{
  ...
  $choice = (isset($_GET['search']) && $_GET['search'] == "pagerank")? "pagerank" : "default";
...

  {
    ...
    for($i=0;$i<sizeOf($word);$i++){
      ...
      if($guess!="")
        ...
        $guess1 = $guess1." ".trim($che);

    }

    $guess1 = str_replace("+"," ",$guess);

    $div=false;

    if(strtolower($query)==strtolower($guess1)){
      ...
      $link = "http://localhost/irhw5.php?q=$guess&sort=$choice";

      $output = "Did you mean: <a href='$link'>$guess1</a>";
...
<html>
  ...
  <body>
     ...
     <h1 align="center"> Solr Search Engine with Spell Correction and Autocomplete </h1><br/>

    <form accept-charset="utf-8" method="get" id="searchform" style="margin-left:700px">

      Search: <input id="q" name="q" type="text" value="<?php echo htmlspecialchars($query, ENT_QUOTES, 'utf-8'); ?>" list="searchresults" placeholder="Search keywords " autocomplete="off"/>

      <datalist id="searchresults"></datalist>

      <input type="hidden" name="spellcheck" id="spellcheck" value="false"> <br><br>

        <input type="radio" name="search" checked="true" <?php if (isset($_GET['search']) && $_GET['search']=="lucene") echo 'checked="checked"';?>  value="lucene" /> Lucene(Default)
...
$csvArray =  array_map('str_getcsv', file('UrlToHtml_mercury.csv'));
...
<?php
...

  {
    ...
    $id = str_replace("/home/sachin/shared/solr-7.5.0/solr-7.5.0/crawl_data/","",$id);
    ...
    $html_to_text_files_dir = "/home/sachin/shared/solr-7.5.0/solr-7.5.0/crawl_data/";
